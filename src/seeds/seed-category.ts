        ...
        if (categoryData.children && categoryData.children.length > 0) {
            ...
            for (const childName of categoryData.children) {
                ...
                if (!existingChild) {
                    ...
                    console.log(`Добавлена дочерняя категория: ${childName} к родительской ${categoryData.name}`);
                    created++;
                } else {
                    console.log(`Дочерняя категория ${childName} уже существует у родительской ${categoryData.name}`);
                }
            }
        }
    }
